...
echo "Installing..."
if [ -f ./files/config/sync/core.extension.yml ] ; then $DRUSH $SITE_ALIAS si manatiblog --config-dir=sites/default/files/config/sync -y ; else $DRUSH $SITE_ALIAS si manatiblog -y ; fi
